...
/datum/component/uplink/proc/MakePurchase(mob/user, datum/uplink_item/U)
	...
	if (!user || user.incapacitated())
		return
	if(U.hijack_only && !(usr.mind.special_role == ROLE_OPERATIVE) || !(usr.mind.special_role == "nukie mid"))//nukies get items that regular traitors only get with hijack. If a hijack-only item is not for nukies, then exclude it via the gamemode list.
