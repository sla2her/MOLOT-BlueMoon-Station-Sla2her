...
//		return
//
//	RegisterSignal(target, COMSIG_ITEM_ALT_AFTERATTACK , PROC_REF(try_kneecap_target))
//
///datum/element/kneecapping/Detach(datum/target)
//	UnregisterSignal(target, COMSIG_ITEM_ALT_AFTERATTACK)
//
//	return ..()
//
///**
// * Signal handler for COMSIG_ITEM_ALT_AFTERATTACK. Does checks for pacifism, zones and target state before either returning nothing
...
//
//	if(do_after(attacker, 3 SECONDS, target))
