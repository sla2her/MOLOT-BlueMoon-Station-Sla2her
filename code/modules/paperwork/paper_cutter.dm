...
/obj/item/papercutter/suicide_act(mob/user)
	if(storedcutter)
		...
		user.visible_message("<span class='suicide'>[user] repeatedly bashes [src.name] against [user.ru_ego()] head! It looks like [user.ru_who()] trying to commit suicide!</span>")
