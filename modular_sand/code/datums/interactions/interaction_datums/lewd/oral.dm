...
/datum/interaction/lewd/oral/display_interaction(mob/living/user, mob/living/partner)
	...
	if(partner.is_fucking(user, CUM_TARGET_MOUTH))
		if(prob(partner.get_sexual_potency()))
			...
			message = "становится всё проворней и проворней в случае с \the <b>[partner]</b>."
			lust_increase += 5
		else
			var/improv = FALSE
			switch(fucktarget)
	...
	if(fucktarget != "penis" || partner.can_penetrating_genital_cum())
		partner.handle_post_sex(NORMAL_LUST, CUM_TARGET_MOUTH, user, ORGAN_SLOT_PENIS) //SPLURT edit
	else
		partner.handle_post_sex(NORMAL_LUST, CUM_TARGET_MOUTH, user, ORGAN_SLOT_VAGINA)
